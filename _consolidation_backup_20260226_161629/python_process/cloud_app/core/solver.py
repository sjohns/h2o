...
from .lcm import calculate_bundles_per_truckload_value, calculate_lcm_from_rows
from .ratios import calculate_target_weights, score_solution


def _build_solver_rows(df: pd.DataFrame) -> list[dict]:
    rows: list[dict] = []
    for row_values in df.to_dict(orient="records"):
        sticks_per_bundle = max(1, int(row_values["calculated_sticks_per_bundle"]))
        bundles_per_truckload = max(
            1,
            calculate_bundles_per_truckload_value(
                eagle_sticks_per_truckload=int(row_values["eagle_sticks_per_truckload"]),
                calculated_sticks_per_bundle=sticks_per_bundle,
            ),
        )
        bundle_size = 0
        row = {
            "sku_id": str(row_values["sku_id"]),
            "sku": str(row_values["sku"]),
            "popularity_score": max(1, int(row_values["popularity_score"])),
...
def solve(
    ...
    working_df = working_df[working_df["sku_id"].isin(list(selected))]
